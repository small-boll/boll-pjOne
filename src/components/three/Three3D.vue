...
export default {
  ...
  methods: {
    init () {
       ...
       * 创建网格模型
       */
      // const geometry = new THREE.SphereGeometry(60, 60, 60); //创建一个球体几何对象
      ...
      document.getElementById('threeContain').appendChild(this.renderer.domElement)
      //执行渲染操作   指定场景、相机作为参数
      // renderer.render(this.scene, this.camera);
      // this.timer = setInterval(this.render,20);
      this.renderFn()
    },
    renderFn() {
      this.renderer.render(this.scene,this.camera);//执行渲染操作
      this.mesh.rotateY(0.01);//每次绕y轴旋转0.01弧度
      // 调用一个函数不是立即调用而是向浏览器发起一个执行某函数的请求， 什么时候会执行由浏览器决定，一般默认保持60FPS的频率，大约每16.7ms调用一次requestAnimationFrame()方法指定的函数
      requestAnimationFrame(this.renderFn);//请求再次执行渲染函数render
    }
  },
  beforeDestroy () {
